...
The Windows package is written to `artifacts\package`. macOS packages are built and verified by the GitHub Actions workflow `.github/workflows/build-macos-release.yml`; no Mac development machine is required. Run `Build signed macOS release packages` from the Actions tab and provide the release tag to update. The workflow builds both `osx-x64` and `osx-arm64`, disables debug symbols, signs the complete app bundle with Developer ID, notarizes it, staples the ticket, extracts the ZIP again, and verifies Gatekeeper acceptance before uploading. An empty `release_tag` is for architecture/extraction checks only and produces an ad-hoc artifact that may be blocked when downloaded.
