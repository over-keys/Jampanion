...
if ! command -v node >/dev/null 2>&1 && command -v brew >/dev/null 2>&1 && brew --prefix node@22 >/dev/null 2>&1; then
  export PATH="$(brew --prefix node@22)/bin:$PATH"
fi
command -v node >/dev/null 2>&1 || fail 'Node.js 20 or later is required.'
command -v npm >/dev/null 2>&1 || fail 'npm is required.'
NODE_MAJOR="$(node -p 'Number(process.versions.node.split(`.`)[0])')"
[[ "$NODE_MAJOR" -ge 20 ]] || fail "Node.js 20 or later is required; found $(node -v)."
