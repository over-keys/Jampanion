...
MIDI input and output are optional. The built-in Trio is the default output. A manually selected external MIDI output is remembered and used when it is available.

Imported or edited charts are plain-text `.cho` files in `Documents\Jampanion\Songs`.

On macOS, open `Jampanion.app`. Releases are Developer ID signed and notarized when the required Apple credentials are configured. Otherwise, macOS may require one-time approval through Privacy & Security > Open Anyway.
