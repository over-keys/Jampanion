...
Check(
    "latin stage names use neutral energy labels",
    Enum.GetNames<LatinChorusStage>().SequenceEqual(
        new[] { "Opening", "Groove", "Developing", "Peak", "HeadOut" }));
...
var countIn = SessionPlanBuilder.BuildCountIn(TuneCatalog.Default, 120);
Check(
    "count-in remains below the accompaniment ceiling",
    countIn.Notes.Count > 0 &&
    countIn.Notes.All(note => note.Velocity <= 80));
Check(
    "count-in retains a clear downbeat accent",
    countIn.Notes.Count > 1 &&
    countIn.Notes[0].Velocity > countIn.Notes[1].Velocity);
...
var waltz = TuneCatalog.All.First(tune => tune.BeatsPerBar == 3);
var waltzBars = waltz.Bars.Take(4).ToArray();
var waltzArrangements = Enumerable.Range(0, waltzBars.Length)
    .Select(index => new BarArrangement(
        index,
        ResponderRole.Structural,
        PhraseFunction.Build,
        IsSectionEnding: index == waltzBars.Length - 1,
        Boundary: index == waltzBars.Length - 1
            ? BoundaryStrength.Section
            : BoundaryStrength.None))
    .ToArray();
Check(
    "automatic jazz-waltz hemiola stays disabled",
    !WaltzHemiolaPlanner.Plan(
        waltzBars,
        waltzArrangements,
        seed: 17,
        stage: WaltzChorusStage.Lifted).IsActive);
